...
def analytical_FC(y, Q) : 
    if(y!=0) :return Q * 1/2 * np.sqrt(2/np.pi) /np.sqrt(np.pi/2) * 1/(2*y*y) * (np.pi*math.erf(y/np.sqrt(2))-np.sqrt(2*np.pi)*y * np.exp(-y*y/2)) 
    else: return 0
...
def main(): 
    ...
    DIR = "integral0.5_l"+str(l)+"_gamma_" + str(gamma) +"_Q_0_C_" + str(C)
    IMAGE_DIR = DIR+ "/" + "plots"
    FILE_DIR = DIR+ "/" + "files"
    ...
    x_aux = np.linspace(0,10, 4096)
    Fc = ForceFFT.GetCollectiveForce(x_aux, initial(x_aux))
    analytical = []
    for i in x_aux:
        analytical.append(analytical_FC(i,1))
    plt.plot(x_aux, Fc, label="Fc")
    plt.plot(x_aux, np.array(analytical), label= "anal")
    plt.ylim(-5,1)
    plt.legend()
    plt.show()
