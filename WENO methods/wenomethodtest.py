...
v = zeros(nx)
u[:]=np.exp(-(x)*(x)/0.01) #setting u = 2 between 0.5 and 1 as per our I.C.s
v[:]=2 #setting u = 2 between 0.5 and 1 as per our I.C.s
...
for n in range(0,nt):  
    ...
    if(n%NIterationImage==0):
        plt.cla()
        #plt.xlim(-1,1)
        #plt.ylim(-0.05,1.05)
        plt.plot(x, uc[gc:nx+gc], color='black', linewidth=1.5)
        plt.plot(x, np.exp(-(x-c*n*dt)*(x-c*n*dt)/0.01), linewidth=1.5)
        plt.title(str(round(n*dt,3)))
        plt.pause(0.001)